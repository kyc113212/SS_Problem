...
int main() {
	...
	for (int tc = 1; tc <= T; tc++) {
		...
		for (int m = 0; m < M; m++) {
			...
			int etempB = 0;
			if (A == 1) {
				if (visited[0][ay][ax] == 1 && visited[0][by][bx] == 1) {
					tempA = map[0][ay][ax] / 2;
					tempB = map[0][by][bx] / 2;
				}
				else {
				...
				for (int u = 0; u < A; u++) {
					for (int w = 0; w < A; w++) {
						if (u != w) {
							...
							if (temp_sum < map[u][ay][ax] / 2 + map[w][by][bx] / 2) {
								eAidx = u;
								eBidx = w;
								etempA = map[u][ay][ax] / 2;
								etempB = map[w][by][bx] / 2;
								temp_sum = etempA + etempB;
							}
						}
					}
				}
